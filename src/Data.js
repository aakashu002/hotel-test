...
export class Data extends Component {
  ...
  displayHotelList = () => {
    let newArr= this.state.Hotel.map((item, index) => {
      return (
        <li key={item.id} className="">
          <Link to={{pathname:'/Resturent', state: {item}}}>
            {item.name}
          </Link>
        </li>
      );
    ...
    return (
      <div>
        ...
        <ul>
          ...
          {/* <h3>List of Hotel</h3>
          <div class="">
            <Link class="" to="/Resturent" onClick={this.clickMe}>
              Mission Chinese Food
            </Link>
            <Link class="" to="/Resturent">
              Emily
            </Link>
            <Link class="dropdown-item" to="/Resturent">
